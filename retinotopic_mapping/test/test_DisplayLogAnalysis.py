...
class TestDisplayLogAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        self.log_path1 = os.path.join(curr_folder, 'test_data',
                                      '180202152841-CombinedStimuli-MMOUSE-USER-TEST-notTriggered-complete.pkl')
        self.log_path2 = os.path.join(curr_folder, 'test_data',
                                      '180310165253-CombinedStimuli-MMOUSE-USER-TEST-notTriggered-complete.pkl')

    def test_DisplayLogAnalyzer(self):
        log = dla.DisplayLogAnalyzer(log_path=self.log_path1)
    ...
    def test_DisplayLogAnalyzer_LSN(self):
        log = dla.DisplayLogAnalyzer(log_path=self.log_path2)
        stim_dict = log.get_stim_dict()
        ...
        repeat = stim_dict['001_LocallySparseNoiseRetinotopicMapping']['repeat']
        iteration = stim_dict['001_LocallySparseNoiseRetinotopicMapping']['iteration']

        lsn_dict = pd_onsets_com['001_LocallySparseNoiseRetinotopicMapping']
        # print('\n'.join(lsn_dict.keys()))
