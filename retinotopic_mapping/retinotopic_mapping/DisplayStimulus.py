...
def analyze_frames(ts, refresh_rate, check_point=(0.02, 0.033, 0.05, 0.1)):
    ...
    Calculates relevant statistics of the images frames containing
    the stimulus presentation, particularly in order  for timing 
    purposes.
    
    Parameters
    ----------
    ts : ndarray
        list of time stamps of each frame (in seconds).
    refresh_rate : float
        the refresh rate of imaging monitor (in Hz).
    check_point : tuple, optional
        list of times where each element is used to check how many 
        frames have been presented that are longer than the respective 
        element.

    Returns
    -------
    frame_duration : ndarray
        list containing the length of each time stamp (in seconds).
    frame_stats : str
...
class DisplaySequence(object):
    """
    Displays the numpy sequence from memory.
    
    Parameters
    ----------
    log_dir : str
        system directory path to where the experimental log data will be saved.
    backupdir : str, optional
        copy of directory path to save a backup of the experimental log data, 
        defaults to `None`.
    display_iter : int, optional
        number of iterations a specific stimulus routine will be presented, 
        defaults to `1`.
    ...
    psychopy_mon : str, optional
        label for the monitor used for displaying the stimulus, defaults to 
        'testMonitor'.
    is_interpolate : bool, optional
        determines whether or not PsychoPy will interpolate pixels during a 
        downsampling of the monitor pixels, defaults to `False`.
    ...
    trigger_NI_dev : str, optional
        label for National Instruments hardware device connected to trigger
        the beginning of an experiment, defaults to 'Dev1'.
    trigger_NI_port : int, optional
        port number for National Instruments hardware device connected to 
        trigger the beginning of an experiment, defaults to `1`.
    trigger_NI_line : int, optional
        line of National Instruments hardware device connected to experiment,
        defaults to `0`.
    is_sync_pulse_pulse : bool, optional
        determines if a synchronized TTL signal is used for fine scaled experimental
        timing; defaults to `True`.
    sync_pulse_NI_dev : str, optional
        the label for National Instruments device, efaults to 'Dev1'.
    sync_pulse_NI_port : int, optional 
        the port for National Instuments device, defaults to 1.
    sync_pulse_NI_line : int, optional
        the line for National Instruments device, defaulsts to 0.
    ...
    def flag_to_close(self):
        """ Determine if stimulus presenetation routine has finished """
    ...
    def set_display_order(self, display_order):
        """ set order in which stimulus is presented """
        
        self.display_order = display_order
        self.clear()
    
    def set_display_iteration(self, display_iter):
        """ set number of times full stimulus routine is presented """
        if display_iter % 1 == 0:
            self.display_iter = display_iter
        else:
            raise ArithmeticError, "`display_iter` should be a whole number."
